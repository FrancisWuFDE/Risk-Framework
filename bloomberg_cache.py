...
def _load_blpapi() -> Any:
    """Import Bloomberg's Python API or raise an actionable error."""
    try:
...
def _bloomberg_security(ticker: str) -> str:
    """Return a Bloomberg security identifier for a portfolio ticker."""
...
def _normalize_payload(value: Any) -> list[Any]:
    """Return a Bloomberg response object as a list."""
    if value is None:
        return []
...
def _as_float(value: Any) -> float:
    """Convert a Bloomberg field value to float or NaN."""
    try:
        numeric = float(value)
...
def _reference_data_session() -> Iterator[tuple[Any, Any, Any]]:
    """Yield a running Bloomberg reference-data session."""
    blpapi = _load_blpapi()
...
def _send_request(
    ...
    """Send one synchronous request and return decoded response messages."""
...
def ensure_price_history(
    ...
    for ticker in unique_tickers:
        ...
        if start_date < minimum_date:
            requests_by_range.setdefault(
                (start_date, min(end_date, minimum_date - timedelta(days=1))),
                [],
            ).append(ticker)
        if end_date > maximum_date:
            requests_by_range.setdefault(
                (max(start_date, maximum_date + timedelta(days=1)), end_date),
                [],
            ).append(ticker)
...
def _download_market_cap_batch(
    ...
    for record in records:
        ...
        if not math.isnan(value_in_millions) and value_in_millions > 0:
            market_caps.loc[record["ticker"]] = value_in_millions * 1_000_000
    return market_caps
